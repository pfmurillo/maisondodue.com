<script lang="ts">
	...
	onMount(() => {
		...
		document.addEventListener('scrollend', (event) => {
			let scrollDelta = window.scrollY - prevScrollY;
			if (scrollDelta > 200) {
				showHeader = false;
			} else if (scrollDelta < -200 || window.scrollY < 200) {
				showHeader = true;
			}
...
<header class:hideHeader={!showHeader && !showNav}>
	<nav aria-label={$_('header.arialabel')}>
		...
		<ul class:showNav>
			...
			<li>
				<a on:click={hideNav} href="https://instagram.com/maisondodue" title="Instagram"
					><svg
	...
	<button class:showNav on:click={toggleNav} aria-label={$_('aria.toggleNav')} />
</header>
...
<style lang="scss">
	header {
		...
		padding: 12px $root-horizontal-spacing;
	...
	.home > span:last-of-type {
		...
		background: linear-gradient(12deg, hsla(321, 42%, 48%, 1) 32%, hsla(173, 80%, 65%, 1) 100%);
		color: transparent;
	...
	@media (max-width: $breakpoint-mobile) {
		...
		button::after {
			...
			width: 44px;
			height: 44px;
			background: linear-gradient(transparent 47%, $color-accent 47% 53%, transparent 53%);
		...
		button.showNav::before,
		button.showNav::after {
