<script lang="ts">
	...
	let solidBackground = $state(false);
	...
	onMount(() => {
		...
		window.addEventListener('click', (event) => {
			...
			if (!nav.contains(node) && !header.contains(node)) {
				showNav = false;
			}
		});
	});
...
>
	<ul
		class:text-black={solidBackground}
		class="mx-3 my-3 flex items-center transition-all md:mx-14 md:my-6"
	>
		<li class="grow">
			<a href="/">Maison Dodue</a>
		</li>
		<li class="mr-3 md:mr-8">
			<button class="flex" onclick={toggleNav}>
				<span class="max-sm:hidden">{m.great_major_llama_delight()}</span>
				<svg
		...
		<li>
			<a href="https://instagram.com/maisondodue" target="_blank" aria-label="Instagram"
				><svg
					viewBox="0 0 256 256"
					version="1.1"
					xmlns="http://www.w3.org/2000/svg"
	...
	{#if showNav}
		<nav transition:slide class="fixed z-20 w-full bg-white shadow-lg">
			...
			>
				...
				{/each}
				<li class="flex items-center justify-center sm:hidden">
					<NavSection>
						<button
							onclick={() => {
								scrollTo('footer');
